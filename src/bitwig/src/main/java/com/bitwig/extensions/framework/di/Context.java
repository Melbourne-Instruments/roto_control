...
public class Context {
     ...
     */
    public Context(final ControllerExtension extension) {
        initializeExtension(extension);
        scanPackage(extension.getClass());
    }
    ...
    private record ComponentClassBind(Class<?> clazz, Component component) implements Comparable<ComponentClassBind> {
        //
        static Optional<ComponentClassBind> create(Class<?> clazz) {
        ...
        @Override
        public int compareTo(ComponentClassBind o) {
            return o.component.priority() - component.priority();
        }
    }

    private void scanPackage(final Class<?> baseClass) {
        try {
            final List<Class<?>> classes = PackageHelper.getClasses(baseClass);
            List<ComponentClassBind> components = classes.stream()//
                    .map(ComponentClassBind::create) //
                    .flatMap(o -> o.stream()).sorted() //
                    .toList();
            for (ComponentClassBind bind : components) {
    ...
    private void tryToCompleteIncompleteSetter(final Class<?> newType) {
        ...
        while (it.hasNext()) {
            final ComponentClosure<?> closure = it.next();
            if (closure.instance != null && closure.missingSetters.remove(
                    newType) && closure.missingSetters.isEmpty()) {
                try {
    ...
    private void injectBySetter(final Object object,
                                final ComponentClosure<?> closure) throws IllegalAccessException, InvocationTargetException {
        final Method[] methods = closure.clazz.getMethods();

        for (final Method method : methods) {
            final Inject injectNotation = method.getAnnotation(Inject.class);
            if (injectNotation != null && !Modifier.isPrivate(
                    method.getModifiers()) && method.getParameterCount() == 1) {
    ...
    private <T> void invokePostConstruct(final T object,
    ...
    private <T> Object[] getConstructableArguments(final Constructor<T> constructor, final Component compAnnotation) {
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            ...
            if (constructorObject == null && paramType == String.class && !foundName && compAnnotation != null && !compAnnotation.name()
                    .isBlank()) {
    ...
    private <T> boolean inject(final T object,
